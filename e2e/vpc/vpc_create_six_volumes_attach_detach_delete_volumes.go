...
var _ = Describe("ibmcloud-storage-volume-lib", func() {
	...
	It("VPC: Parallel volume attachments, detachments [six volume attachments in parallel]", func() {
		...
		ctxLogger.Info("Creating test volumes", zap.Reflect("Elasped time:", time.Since(startTime)))
		...
		ctxLogger.Info("Testing parallel attach volumes", zap.Reflect("Elasped time:", time.Since(startTime)))
		...
		ctxLogger.Info("Test parallel detach Volumes", zap.Reflect("Elasped time:", time.Since(startTime)))
		...
		ctxLogger.Info("Test delete volumes", zap.Reflect("Elasped time:", time.Since(startTime)))
		fmt.Printf("\n\n")
	})
})
